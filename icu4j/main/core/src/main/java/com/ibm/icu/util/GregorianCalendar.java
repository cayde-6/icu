 ...
 *
 * <p><code>GregorianCalendar</code> is a concrete subclass of {@link Calendar} and provides the
 * standard calendar used by most of the world.
 *
 * <p>The standard (Gregorian) calendar has 2 eras, BC and AD.
 *
 * <p>This implementation handles a single discontinuity, which corresponds by default to the date
 * the Gregorian calendar was instituted (October 15, 1582 in some countries, later in others). The
 * cutover date may be changed by the caller by calling <code>setGregorianChange()</code>.
 *
 * <p>Historically, in those countries which adopted the Gregorian calendar first, October 4, 1582
 * was thus followed by October 15, 1582. This calendar models this correctly. Before the Gregorian
 * cutover, <code>GregorianCalendar</code> implements the Julian calendar. The only difference
 * between the Gregorian and the Julian calendar is the leap year rule. The Julian calendar
 * specifies leap years every four years, whereas the Gregorian calendar omits century years which
 * are not divisible by 400.
 *
 * <p><code>GregorianCalendar</code> implements <em>proleptic</em> Gregorian and Julian calendars.
 * That is, dates are computed by extrapolating the current rules indefinitely far backward and
 * forward in time. As a result, <code>GregorianCalendar</code> may be used for all years to
 * generate meaningful and consistent results. However, dates obtained using <code>GregorianCalendar
 * </code> are historically accurate only from March 1, 4 AD onward, when modern Julian calendar
 * rules were adopted. Before this date, leap year rules were applied irregularly, and before 45 BC
 * the Julian calendar did not even exist.
 *
 * <p>Prior to the institution of the Gregorian calendar, New Year's Day was March 25. To avoid
 ...
 *
 * <p>Values calculated for the <code>WEEK_OF_YEAR</code> field range from 1 to 53. Week 1 for a
 * year is the earliest seven day period starting on <code>getFirstDayOfWeek()</code> that contains
 * at least <code>getMinimalDaysInFirstWeek()</code> days from that year. It thus depends on the
 * values of <code>getMinimalDaysInFirstWeek()</code>, <code>getFirstDayOfWeek()</code>, and the day
 * of the week of January 1. Weeks between week 1 of one year and week 1 of the following year are
 * numbered sequentially from 2 to 52 or 53 (as needed).
 *
 * <p>For example, January 1, 1998 was a Thursday. If <code>getFirstDayOfWeek()</code> is <code>
 * MONDAY</code> and <code>getMinimalDaysInFirstWeek()</code> is 4 (these are the values reflecting
 * ISO 8601 and many national standards), then week 1 of 1998 starts on December 29, 1997, and ends
 * on January 4, 1998. If, however, <code>getFirstDayOfWeek()</code> is <code>SUNDAY</code>, then
 * week 1 of 1998 starts on January 4, 1998, and ends on January 10, 1998; the first three days of
 * 1998 then are part of week 53 of 1997.
 *
 * <p>Values calculated for the <code>WEEK_OF_MONTH</code> field range from 0 or 1 to 4 or 5. Week 1
 * of a month (the days with <code>WEEK_OF_MONTH =
 * 1</code>) is the earliest set of at least <code>getMinimalDaysInFirstWeek()</code> contiguous
 * days in that month, ending on the day before <code>getFirstDayOfWeek()</code>. Unlike week 1 of a
 * year, week 1 of a month may be shorter than 7 days, need not start on <code>getFirstDayOfWeek()
 * </code>, and will not include days of the previous month. Days of a month before week 1 have a
 * <code>WEEK_OF_MONTH</code> of 0.
 *
 * <p>For example, if <code>getFirstDayOfWeek()</code> is <code>SUNDAY</code> and <code>
 * getMinimalDaysInFirstWeek()</code> is 4, then the first week of January 1998 is Sunday, January 4
 * through Saturday, January 10. These days have a <code>WEEK_OF_MONTH</code> of 1. Thursday,
 * January 1 through Saturday, January 3 have a <code>WEEK_OF_MONTH</code> of 0. If <code>
 * getMinimalDaysInFirstWeek()</code> is changed to 3, then January 1 through January 3 have a
 * <code>WEEK_OF_MONTH</code> of 1.
 *
 ...
 * </blockquote>
 *
 * <p>GregorianCalendar usually should be instantiated using {@link
 * com.ibm.icu.util.Calendar#getInstance(ULocale)} passing in a <code>ULocale</code> with the tag
 * <code>"@calendar=gregorian"</code>.
 *
 * @see Calendar
 * @see TimeZone
...
public class GregorianCalendar extends Calendar implements Cloneable {
    ...
    /**
     * The point at which the Gregorian calendar rules are used, measured in milliseconds from the
     * standard epoch. Default is October 15, 1582 (Gregorian) 00:00:00 UTC or -12219292800000L. For
     * this value, October 4, 1582 (Julian) is followed by October 15, 1582 (Gregorian). This
    ...
    /**
     * Constructs a GregorianCalendar based on the current time in the default time zone with the
     * given locale.
     *
    ...
    /**
     * {@icu} Constructs a GregorianCalendar based on the current time in the default time zone with
     * the given locale.
     *
    ...
    /**
     * {@icu} Constructs a GregorianCalendar based on the current time in the given time zone with
     * the given locale.
     *
     * @param zone the given time zone.
     * @param aLocale the given locale.
     ...
     * @param month the value used to set the MONTH time field in the calendar. Month value is
    ...
    /**
     * Sets the GregorianCalendar change date. This is the point when the switch from Julian dates
     * to Gregorian dates occurred. Default is October 15, 1582. Previous to this, dates will be in
     * the Julian calendar.
     *
     * <p>To obtain a pure Julian calendar, set the change date to <code>Date(Long.MAX_VALUE)</code>
     * . To obtain a pure Gregorian calendar, set the change date to <code>Date(Long.MIN_VALUE)
     * </code>.
     *
    ...
    /**
     * Gets the Gregorian Calendar change date. This is the point when the switch from Julian dates
     * to Gregorian dates occurred. Default is October 15, 1582. Previous to this, dates will be in
     * the Julian calendar.
     *
            ...
            case WEEK_OF_YEAR:
                {
                    ...
                    if (woy < 1 || woy > 52) {
                        ...
                        int lastRelDow =
                                (lastDoy - isoDoy + internalGet(DAY_OF_WEEK) - getFirstDayOfWeek())
                                        % 7;
    ...
    /**
     * Return the minimum value that this field could have, given the current date. For the
     * Gregorian calendar, this is the same as getMinimum() and getGreatestMinimum().
     *
     * @stable ICU 2.0
     */
    @Override
    ...
    /**
     * Return the maximum value that this field could have, given the current date. For example,
     * with the date "Feb 3, 1997" and the DAY_OF_MONTH field, the actual maximum would be 28; for
     * "Feb 3, 1996" it s 29. Similarly for a Hebrew calendar, for some years the actual maximum for
    ...
    /**
     * Override Calendar to compute several fields specific to the hybrid Gregorian-Julian calendar
     * system. These are:
     *
     * <ul>
     *   <li>ERA
